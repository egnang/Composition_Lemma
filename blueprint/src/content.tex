...
\end{definition}
\begin{definition}[Automorphism group]\label{defn:aut-functional-graphs}
  %\lean{}
  %\leanok
  %\uses{defn:functional-directed-graphs}
...
\end{proof}
\begin{definition}[Polynomial of Grace]\label{defn:polynomial-grace-definition} We define $P_f \in \mathbb{C}[x_0, \ldots, x_{n-1}]$ for all $f \in \mathbb{Z}_n^{\mathbb{Z}_n}$ as follows:
\begin{equation}
    P_f \,:= \underbrace{\prod_{0\le u<v<n}(x_{v}-x_{u})}_{V_f(x_0,\ldots,x_{n-1})}\,\underbrace{\prod_{0\le u<v<n}\big((x_{f(v)}-x_{v})^{2}-(x_{f(u)}-x_{u})^{2}\big)}_{E_f(x_0,\ldots,x_{n-1})}.
\end{equation}
\end{definition}
\begin{definition}[Congruence class]\label{defn:polynomial-congruence}
  %\lean{}
  %\leanok
...
\begin{proposition}[Composition Inequality]\label{prop:composition-lemma-ineq}
  %\lean{}
  %\leanok
